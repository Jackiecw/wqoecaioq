          ...
          <div class="space-y-4 mb-8">
            ...
            <div>
              ...
              <textarea
                ...
                rows="2"
                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none"
                placeholder="补充模板适用范围、评分规则等..."
              ></textarea>
            </div>
          </div>

          <div class="mb-4 flex justify-between items-center">
            <h3 class="text-lg font-semibold text-gray-800">指标项 (KPI)</h3>
            <div class="flex items-center gap-4">
    ...
    <div v-if="showPreviewModal" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4">
      <div class="bg-white rounded-2xl shadow-xl w-full max-w-3xl max-h-[90vh] flex flex-col">
        ...
        <div class="p-6 overflow-y-auto flex-1">
          ...
          <h3 class="font-semibold text-gray-800 mb-4">指标项</h3>
          <div class="space-y-4">
...
const selectedTemplate = ref<PerformanceTemplate | null>(null);
...
const form = ref<{
  ...
  description: '',
  items: [
    { category: '通用', kpiName: '', description: '', weight: 0 },
  ],
});

const totalWeight = computed(() => form.value.items.reduce((sum, item) => sum + (Number(item.weight) || 0), 0));

const fetchTemplates = async () => {
  try {
    const res = await performanceService.getTemplates();
    templates.value = res || [];
  } catch (error) {
...
const openPreviewModal = (template: PerformanceTemplate) => {
...
const submitTemplate = async () => {
  ...
  try {
    await performanceService.createTemplate(form.value);
